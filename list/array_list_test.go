/**
 * Description：
 * FileName：array_list_test.go.go
 * Author：CJiaの用心
 * Create：2025/9/21 22:43:58
 * Remark：
 */
...
func TestArrayList_Get(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			index:   2,
			wantVal: 0,
			wantErr: fmt.Errorf("echo: 下标超出范围，长度 %d, 下标 %d", 2, 2),
		},
		{
			name:    "index -1",
			list:    NewArrayListOf[int]([]int{123, 100}),
			index:   -1,
			wantVal: 0,
			wantErr: fmt.Errorf("echo: 下标超出范围，长度 %d, 下标 %d", 2, -1),
		},
	}
...
func TestArrayList_Add(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			newVal:  100,
			index:   -1,
			wantErr: fmt.Errorf("echo: 下标超出范围，长度 %d, 下标 %d", 3, -1),
		},
		{
			...
			newVal:  100,
			index:   4,
			wantErr: fmt.Errorf("echo: 下标超出范围，长度 %d, 下标 %d", 3, 4),
		},
	}
...
func TestArrayList_Set(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			newVal:    5,
			wantSlice: []int{},
			wantErr:   fmt.Errorf("echo: 下标超出范围，长度 %d, 下标 %d", 5, -1),
		},
		{
			name:      "index 100",
			list:      NewArrayListOf[int]([]int{0, 1, 2, 3, 4}),
			...
			newVal:    5,
			wantSlice: []int{},
			wantErr:   fmt.Errorf("echo: 下标超出范围，长度 %d, 下标 %d", 5, 100),
		},
	}
